...
#   WATCH='80097020,8008CCE0:16' ./run.sh -> avisa quando essas regioes mudam
#   POKE='8008CCE2:2|1' ./run.sh -> segura um valor na memoria todo quadro
#   PADWATCH=1 ./run.sh     -> mostra a janela do pad sempre que ela mudar
#   GOWATCH=1 ./run.sh       -> monitora tudo que muda durante a tela GAME OVER
#   STEPWATCH=1 ./run.sh    -> despeja os 4 slots de seta a cada mudanca
#   AUTOPLAY=1 ./run.sh     -> o jogo pisa sozinho (PERFECT em tudo)
...
[ -n "${WATCH:-}" ] && export PSX_WATCH="$WATCH"
[ -n "${POKE:-}" ] && export PSX_POKE="$POKE"
[ -n "${PADWATCH:-}" ] && export PSX_PADWATCH="$PADWATCH"
[ -n "${GOWATCH:-}" ] && export PSX_GOWATCH="$GOWATCH"
[ -n "${STEPWATCH:-}" ] && export PSX_STEPWATCH="$STEPWATCH"
[ -n "${AUTOPLAY:-}" ] && export PSX_AUTOPLAY="$AUTOPLAY"
